    ...
    <div id="conteudo" class="container p-0 my-4">
        ...
        <div class="d-flex my-1">
            <div id="box1" class=" form-floating w-50 me-1">
                {{ Form::text('name',$posto->name, ['class' => 'form-control', 'id'=> 'name', 'placeholder' => 'nome do posto']) }}
                {{ Form::label('nome do posto', null, ['for'=>'name']) }}
            </div>

            <div id="box2" class="form-floating w-50">
                {{ Form::text('cnpj',$posto->cnpj, ['class' => 'form-control ', 'id'=> 'cnpj', 'placeholder' => 'CNPJ'] ) }}
        ...
        <div class="d-flex my-1">
            ...
            <div id="box5" class="form-floating w-50">
                {{ Form::text('tel', $posto->tel, ['class' => 'form-control', 'id'=> 'tel' , 'placeholder' => 'Tel. Comercial']) }}
                {{ Form::label('Tel. Comercial', null, ['for'=>'tel']) }}
            </div>
        </div>

        <div id="box6" class=" form-floating my-1">
            {{ Form::text('address',$posto->address,  ['class' => 'form-control', 'id'=> 'address' , 'placeholder' => 'Endereço'] ) }}
            {{ Form::label('Endereço', null, ['for'=>'address']) }}
        </div>

        <div class="d-flex">
            <div id="box7" class="form-floating w-50 me-1">
                {{ Form::select('contrato_id', $contratos, $posto->contrato->id,array('class' => 'form-select', 'id'=>'contrato')) }}
                {{ Form::label('contrato_id', 'Contrato', ['for'=>'contrato']) }}
            </div>

            <div  id="box8" class="form-floating w-50">
                {{ Form::select('gerente_id', $users, $posto->gerente_id, array('class' => 'form-select', 'id'=>'gerente' )) }}
