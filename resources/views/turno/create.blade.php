...
@section('content')
    ...
    <div id="conteudo" class="container p-0 my-4">
        ...
        <h4 class="bg-royal-blue text-magnolia fw-normal rounded p-2">Cadastrar Turno</h4>
        
        <div class="d-flex my-1">
            <div id="box1" class="form-floating w-50 me-1">
                {{ Form::select('posto_id',$postos,null,array('class' => 'form-select', 'id'=>'posto') )}}
                {{ Form::label('posto', null, ['for'=>'posto']) }}
            </div>
            <div id="box2" class="form-floating w-50 me-1">
                {{ Form::select('name', ['manha' => 'Manhã', 'tarde' => 'Tarde', 'noite' => 'Noite','madrugada' => 'Madrugada'], null,array('class' => 'form-select', 'id'=>'name') )}}
                {{ Form::label('turno', null, ['for'=>'name']) }}
            </div>
        </div>

        <div class="d-flex my-1">
            <div id="box3" class="form-floating w-50 me-1">
                {{ Form::time('horario_inicio', null, ['class' => 'form-control', 'id'=> 'hri' , 'placeholder' => 'inicio do turno']) }}
                {{ Form::label('Inicio do Turno',null, ['for'=>'hri'] ) }}
            </div>
            <div id="box4" class="form-floating  w-50">
                {{ Form::time('horario_fim', null, ['class' => 'form-control', 'id'=> 'hrfm' , 'placeholder' => 'fim do turno']) }}
                {{ Form::label('fim do turno',null, ['for'=>'hrfm'] ) }}
            </div>
        </div>
