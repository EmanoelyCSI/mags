...
@section('content')
 ...
 <div id="conteudo" class="container">
    ...
    <div id="box" class="d-flex mx-4 my-2 fs-5 justify-content-center">
            <ul>
                <li class="list-group">Criação:              {{ Carbon\Carbon::parse($leitura->create_at)->format('d/m/Y H:i') }} </li class="list-group">
                <li class="list-group">Última modificação:   {{ Carbon\Carbon::parse($leitura->update_at)->format('d/m/Y H:i') }} </li class="list-group">
                {{-- <li class="list-group">Posto:                {{ $leitura->posto-> }}  </li> --}}
                {{-- <li class="list-group">Bomba:                {{ $leitura->bomba->name}}     </li> --}}
                <li class="list-group">Bico:                 {{ $leitura->bico->name}}     </li>
                <li class="list-group">Turno:                {{ $leitura->turno->name}}   </li>
                <li class="list-group">Leitura:              {{ $leitura->leitura  }}     </li>
        
            </ul>
        </div>
            <div id="box" class="d-flex w-25 float-end">
                    <!-- boão de editar-->
                    <a class="text-decoration-none" href="{{ URL::to('leitura/' . $leitura->id . '/edit') }}">
                    ...
                    <!-- botão de excluir -->
                    {{ Form::open(array('url' => 'leitura/' . $leitura->id, 'onsubmit' => 'return ConfirmDelete()'))}}     
