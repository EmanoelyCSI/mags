...
@section('content')
 ...
 <div id="conteudo" class="container">
    ...
    <div id="box" class="d-flex mx-4 my-2 fs-5 justify-content-center">
        <ul>
            <li class="list-group">ID:                   {{ $bomba->id    }}     </li>
            <li class="list-group">Nome:                 {{ $bomba->name  }}     </li>
            <li class="list-group">Modelo:               {{ $bomba->modelo }}     </li>
            <li class="list-group"> Total de bicos:      {{ $bomba->bico->count() }}</li>
            <li class="list-group">Criação:              {{ Carbon\Carbon::parse($bomba->create_at)->format('d/m/Y H:i') }} </li>
            <li class="list-group">Última modificação:   {{ Carbon\Carbon::parse($bomba->update_at)->format('d/m/Y H:i') }} </li>

        </ul>
        </div>

<div class="container w-50">
        <div id="box" class="d-flex  justify-content-around">
                <a href="{{ URL::to('bico/') }}" class="btn bg-navy-blue text-magnolia my-2 me-4 ms-4">Bicos</a>
        <!-- Botões para editar e excluir -->
        <div id="box" class="d-flex">
            <!-- botão de editar-->
            <a class="text-decoration-none align-self-center" href="{{ URL::to('bomba/' . $bomba->id . '/edit') }}">
            ...
            <!-- botão de excluir -->
            {{ Form::open(array('url' => 'bomba/' . $bomba->id, 'onsubmit' => 'return ConfirmDelete()','class' => 'align-self-center' ))}}     
